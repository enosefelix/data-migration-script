...
async function processRecord(
  ...
  try {
    ...
    try {
      await createLotStatement(memberInfo, max_lot_no, record, claimRemark);
    } catch (error) {
    ...
    const { networkId } =
      (await getProviderNetworkId(record.serviceProvider, ms_plan_network)) ||
      0;
...
async function createLotStatement(memberInfo, lot_no, record, claimRemark) {
  // create new statement
  console.log("Creating new statement...");

  const provider = await getProviderInfo(record.serviceProvider);
  try {
    const [result] = await db.query("INSERT INTO lots SET ?", {
      lot_no: lot_no,
      ...
      lot_from_date: null,
      lot_to_date: null,
      lot_type: record.typeOfVisit, // in patient or out patient
...
async function insertAuditAndDataEntry(lot_no) {
  try {
    ...
    await db.query(`INSERT INTO assign_lots SET ?`, {
      al_lot_no: lot_no,
      al_user_id: 1,
      al_action: "D",
...
async function insertClaim(
  ...
  try {
    ...
    const [result] = await db.query("INSERT INTO claim SET ?", {
...
async function storeItemsInTables(
  ...
  try {
    ...
    for (let index = 0; index < items.item.length; index++) {
      ...
      const [tariffData] = await db.query(labQuery, [
        networkId,
        ipOp,
        claimType,
      ]);
    ...
    for (let index = 0; index < items.item.length; index++) {
      ...
      if (drugData.length === 0) {
        ...
        const [globalDrugData] = await db.query(globalDrugQuery);
        drug = globalDrugData;
    ...
    await removeDuplicateDiagnosis(connection, claim_id);
    ...
    try {
      ...
      await updateClaim(labItems, drugItems, claimId, items);
    } catch (e) {
...
async function updateClaim(labItems, drugItems, claimId) {
  ...
  const [result] = await db.query(`UPDATE claim SET ? WHERE claim_id = ?`, [
    {
...
async function insertDrugItems(drugItems, claimId, memberInfo) {
  const unitDrops = await getUnitDrops();
...
async function getUnitDrops() {
  try {
    const unitDrops = await db.query(
